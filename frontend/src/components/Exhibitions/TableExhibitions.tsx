...
import { loadColumns } from './configureExhibitionsCols';
...
const TableSampleExhibitions = ({
  ...
  const {
    ...
    loading,
    count,
    notify: exhibitionsNotify,
    refetch,
  } = useAppSelector((state) => state.exhibitions);
  ...
  useEffect(() => {
    if (exhibitionsNotify.showNotification) {
      notify(
        exhibitionsNotify.typeNotification,
        exhibitionsNotify.textNotification,
      );
    }
  }, [exhibitionsNotify.showNotification]);
  ...
  const handleViewAction = (id: string) => {
    router.push(`/exhibitions/exhibitions-view/?id=${id}`);
  };

  const handleCreateEventAction = ({ start, end }: SlotInfo) => {
    router.push(
      `/exhibitions/exhibitions-new?dateRangeStart=${start.toISOString()}&dateRangeEnd=${end.toISOString()}`,
    );
  };
...
export default TableSampleExhibitions;
